...
def TestGrammar(peg=None):
    ...
    peg.DET_AB = pe('b') | pe('ab')
    peg.AMB_ABB = (pe('a') | pe('ab')) & pe('b')
    peg.Rec =  pe('b') & N % 'Rec' | pe('b')
...
def NLPGrammar(peg=None):
    if peg == None:
        peg = PEG('nlp')
    peg.Assign = TreeAs('Assign', N % '$VARIABLENAME' & pe(
        'を') & N % '$Expression' & Range('とおく', 'とする'))
    peg.Const = TreeAs('Const', N % '$VARIABLENAME' & pe('は')
                       & N % '$Expression' & Range('である'))
