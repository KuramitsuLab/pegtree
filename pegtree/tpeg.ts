...
export const TPEG = (peg?: any) => {
  ...
  pRule(peg, 'EOS', pOre2(pSeq2(pRef(peg, '_'), pOneMany(pSeq2(pChar(';'), pRef(peg, '_')))), pOneMany(pSeq2(pRef(peg, '_'), pRef(peg, 'EOL')))))
  ...
  pRule(peg, 'Expression', pSeq2(pRef(peg, 'Choice'), pOption(pFold('', pOneMany(pSeq(pRef(peg, '__'), pChar('|'), pNot(pChar('|')), pRef(peg, '_'), pEdge('', pRef(peg, 'Choice')))), 'Alt', 0))))
  pRule(peg, 'Choice', pSeq2(pRef(peg, 'Sequence'), pOption(pFold('', pOneMany(pSeq(pRef(peg, '__'), pOre2(pChar('/'), pChar('||')), pRef(peg, '_'), pEdge('', pRef(peg, 'Sequence')))), 'Ore', 0))))
  pRule(peg, 'Sequence', pSeq2(pRef(peg, 'Predicate'), pOption(pFold('', pOneMany(pSeq2(pRef(peg, 'SS'), pEdge('', pRef(peg, 'Predicate')))), 'Seq', 0))))
  pRule(peg, 'SS', pOre2(pSeq3(pRef(peg, 'S'), pRef(peg, '_'), pNot(pRef(peg, 'EOL'))), pSeq3(pOneMany(pSeq2(pRef(peg, '_'), pRef(peg, 'EOL'))), pRef(peg, 'S'), pRef(peg, '_'))))
  ...
  pRule(peg, 'Suffix', pSeq2(pRef(peg, 'Term'), pOption(pOre(pFold('e', pChar('*'), 'Many', 0), pFold('e', pChar('+'), 'OneMany', 0), pFold('e', pChar('?'), 'Option', 0)))))
  ...
  pRule(peg, 'Tag', pSeq2(pChar('#'), pNode(pOneMany(pSeq2(pNot(pRange(' \t\r\n}')), pAny())), 'Tag', 0)))
